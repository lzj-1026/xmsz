<template>
  <div>
      ...
      >
        ...
        >
          ...
          <el-tab-pane label="商品内容" name="4">
            ...
            <!-- 添加商品的按钮 -->
            <el-button class="but_a" type="primary" @click="add"
              >添加商品</el-button
            >
          </el-tab-pane>
        </el-tabs>
    ...
    <el-dialog title="图片预览" :visible.sync="previewVisible" width="50%">
      <img :src="previvewPath" class="i_img" alt="" />
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      onlyTableData: [],
      // 上传图片的api地址
      oploadURl: "http://127.0.0.1:8888/api/private/v1/upload",
      //  图片上传组件的请求头headers请求
      headerObj: {
  ...
  methods: {
    ...
    handlePreview(file) {
      this.previvewPath = file.response.data.url;
    ...
    handleRemove(file) {
      ...
      // 3.调用数组的splice方法，把图片信息对象从pics数组移除
    ...
    add() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (!valid) {
          return this.$message.error("请填写完整内容");
        }
        // 执行添加的业务逻辑
